...
class EmbedPaginator(discord.ui.View):
    ...
    def _update_buttons(self):
        if self.first_page in self.children:
...
class Balls(commands.GroupCog, group_name=settings.balls_slash_name):
    ...
    async def rarity(
        ...
        if not enabled_collectibles:
            await interaction.response.send_message(
                f"There are no collectibles registered in {settings.bot_name} yet.", ephemeral=True
            )
            return
        ...
        if tier:
            ...
            if len(chunks) == 1:
                ...
                for i, chunk in enumerate(chunks):
                    embed = discord.Embed(
                        title=f"{settings.bot_name} Rarity List - T{tier}", color=discord.Color.blurple()
                    )
        ...
        for i, rarity in enumerate(sorted_rarities, 1):
            ...
            names = "\n".join(f"\u200b ⋄ {self.bot.get_emoji(c.emoji_id) or 'N/A'} {c.country}" for c in collectibles)
